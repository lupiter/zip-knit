...
def dump_pattern(
    filename, pattern_number=None, debug=DEBUG, printer=print
...
def __pattern_print(
    ...
    for i in range(99):
        ...
        printer(f"\t{hex(bytenum)}: {hex(unk1)},\t(unknown)")
        bytenum += 1
    ...
    for i in range(99):
        ...
        printer("rows = ", rows, "stitches = ", stitches)
        #        print "total nibs per row = ", nibblesPerRow(stitches)

        # dump the memo data
        printer("memo length =", bytes_for_memo(rows))
        for i in range(bytes_for_memo(rows)):
            b = pointer - i
            printer("\t", hex(b), ": ", hex(bf.get_indexed_byte(b)))
        pointer -= bytes_for_memo(rows)

        printer("pattern length = ", bytes_per_pattern(stitches, rows))
        for i in range(bytes_per_pattern(stitches, rows)):
            b = pointer - i
            printer("\t", hex(b), ": ", hex(bf.get_indexed_byte(b)), end=" ")
            for j in range(8):
